...
class FileSampler:
    ...
    def get_matching_files(self, input_dir: Path, pattern: str = "*", 
    ...
    def stratified_sample(self, input_dir: Path, output_dir: Path, 
        ...
        if not subdirs:
            ...
            sampled = self.sample_directory(input_dir, output_dir, count_per_subdir, pattern)
    ...
    def filter_files_by_size(self, files: list[Path], 
                           min_size: int | None = None,
                           max_size: int | None = None) -> list[Path]:
    ...
    def get_sampling_stats(self, original_files: list[Path], 
                          sampled_files: list[Path]) -> dict[str, int | float]:
        ...
        stats = {
            ...
            "sampling_ratio": len(sampled_files) / len(original_files) if original_files else 0,
