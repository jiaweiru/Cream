...
class IndexMatcher:
                ...
                if source.suffix.lower() == ".json":
                    ...
                    if isinstance(data, dict):
                        for key, value in data.items():
                            content_indices = self.extract_indices_from_content(str(value), pattern)
                            if content_indices:
                                indices[key] = content_indices
                    elif isinstance(data, list):
                        for i, item in enumerate(data):
                            content_indices = self.extract_indices_from_content(str(item), pattern)
                            if content_indices:
                    ...
                    content_indices = self.extract_indices_from_content(content, pattern)
                    if content_indices:
                        indices[source.name] = content_indices
                
                # Also try extracting from filename
                filename_indices = self.extract_indices_from_filename(source.name, pattern)
                if filename_indices:
            ...
            for file_path in source.rglob("*"):
                if file_path.is_file():
                    try:
                        ...
                        filename_indices = self.extract_indices_from_filename(file_path.name, pattern)
                        if filename_indices:
    ...
    def find_matches(self, source_indices: dict[str, list[str]], 
    ...
    def match_indices(self, source: Path, target: Path, pattern: str,
                     match_type: str = "exact") -> list[dict[str, str | float]]:
    ...
    def get_match_statistics(self, matches: list[dict[str, str | float]]) -> dict[str, int | float | dict[str, int]]:
